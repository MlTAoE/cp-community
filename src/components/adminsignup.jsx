...
const AdminSignup = () => {
    ...
    const handleFormSubmit = async (e) => {
        ...
        if (validateEmail(formData.email) && validatePassword(formData.password, formData.confirmPassword)) {
            ...
            try {
                const response = await fetch(import.meta.env.VITE_BACKEND_PORT + 'admin/signup', {
                    method: 'POST',
                    headers: {
                ...
                const { token, admin } = response.ok ? await response.json() : { token: null, admin: null };
    ...
    return (
        <div className='mt-10'>
            <h2 className='text-xl mb-4 font-bold'>Admin Registration</h2>
